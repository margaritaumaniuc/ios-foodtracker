...
// Options for runTest:withBaseline:variations:options:
...
/*
 * This notification is posted whenever an Apptimize
 * recalculates its configuration. The enrolled tests may or may not have changed.
 ...
 * Info.plist: ApptimizeEnableVisualSupport (Boolean, default NO)
 */
...
/**
 Start Apptimize with the provided options, where options is nil or a dictionary
 containing some combination of ApptimizeDevicePairingOption, ApptimizeLogLevelOption,
...
/**
 ...
 provided code template into your code to use this. Options should be nil or a dictionary containing
 ApptimizeUpdateMetadataTimeoutOption and a value in milliseconds.
 For Swift use: runTest:withBaseline:codeBlocks:andOptions:
 ...
 * enrolled in. Note that this does NOT include information about Apptimize A/B tests
 * or Feature Flags that are running but that the device is not enrolled in.
 *
 * @return The NSDictionary whose keys are the names of all A/B tests and Feature Flags
 * the device is enrolled in, and whose values are ApptimizeTestInfo objects containing
 * information about the test or feature flag.
...
/**
 ...
 except by calls to +[Apptimize clearForcedVariant:] or +[Apptimize clearAllForcedVariants].
 
 Note that the "Advanced Verify" feature on the dashboard will not work when there are forced variant(s) on the paired 
 device (because "Advanced Verify" works by sending an instant update to the paired device).
